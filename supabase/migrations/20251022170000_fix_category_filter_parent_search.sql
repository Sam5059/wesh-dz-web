/*
  # Fix category filter to support parent categories

  ## Problem
  When user selects a parent category (e.g., "Véhicules"), no listings are returned
  because listings have category_id pointing to subcategories, not parent categories.

  ## Solution
  Modify search_listings function to:
  1. Check if category_filter is a parent category
  2. If yes, search in all its subcategories
  3. If no, search directly by category_id as before

  ## Changes
  - Update search_listings function to handle parent categories
  - Add logic to find subcategories when filtering by parent
*/

-- Drop existing function
DROP FUNCTION IF EXISTS search_listings(TEXT, UUID, UUID, TEXT, TEXT, NUMERIC, NUMERIC, TEXT);

-- Recreate with parent category support
...
BEGIN
  ...
  -- Si category_filter est fourni, vérifier s'il s'agit d'une catégorie parente
  -- Si oui, récupérer toutes les sous-catégories
  ...
  WHERE
    ...
    -- Filtre catégorie : supporter les catégories parentes ET sous-catégories
    AND (
