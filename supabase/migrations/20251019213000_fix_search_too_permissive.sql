/*
  # Correction de la recherche trop permissive

  Le problème : La recherche retournait des résultats non pertinents
  Solution : Ajouter un score minimum de pertinence requis

  1. Modifications
    - Retirer le bonus des mots individuels de la condition WHERE
    - Filtrer uniquement les résultats avec relevance > 0
    - Rendre la recherche plus stricte en exigeant au moins un match direct
*/

-- Suppression de l'ancienne fonction
DROP FUNCTION IF EXISTS search_listings(TEXT, UUID, UUID, TEXT, TEXT, NUMERIC, NUMERIC, TEXT);

-- Création de la fonction de recherche corrigée et plus stricte
...
BEGIN
  ...
  WITH scored_listings AS (
    SELECT
      ...
      (
        ...
        END
        +
        -- Bonus pour chaque mot trouvé individuellement (seulement si >3 caractères)
        (
          SELECT COALESCE(SUM(
            CASE
    ...
    WHERE
      l.status = 'active'
      AND (
        -- Recherche stricte : au moins un match direct dans titre, description ou attributs
  ...
  SELECT
    scored_listings.id,
    scored_listings.title,
    scored_listings.description,
    scored_listings.price,
    scored_listings.category_id,
    scored_listings.subcategory_id,
    scored_listings.wilaya,
    scored_listings.commune,
    scored_listings.images,
    scored_listings.status,
    scored_listings.listing_type,
    scored_listings.user_id,
    scored_listings.created_at,
    scored_listings.updated_at,
    scored_listings.views_count,
    scored_listings.attributes,
    scored_listings.is_featured,
    scored_listings.condition,
    scored_listings.is_negotiable,
    scored_listings.relevance
