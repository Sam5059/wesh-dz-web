  ...
  SELECT
    ...
    (
      ...
      CASE WHEN l.attributes->>'brand_name' IS NOT NULL
                AND LOWER(l.attributes->>'brand_name') LIKE '%' || LOWER(search_term) || '%' THEN 25 ELSE 0 END
      +
      CASE WHEN l.attributes->>'model_name' IS NOT NULL
                AND LOWER(l.attributes->>'model_name') LIKE '%' || LOWER(search_term) || '%' THEN 25 ELSE 0 END
      +
      CASE WHEN l.attributes IS NOT NULL
                AND LOWER(l.attributes::text) LIKE '%' || LOWER(search_term) || '%' THEN 20 ELSE 0 END
      +
      (
        SELECT COALESCE(SUM(
          CASE WHEN LOWER(l.title) LIKE '%' || w.search_word || '%' THEN 5
               WHEN LOWER(l.description) LIKE '%' || w.search_word || '%' THEN 2
               WHEN l.attributes IS NOT NULL AND LOWER(l.attributes::text) LIKE '%' || w.search_word || '%' THEN 3
               ELSE 0 END
        ), 0)
      ...
      +
      CASE WHEN l.created_at > NOW() - INTERVAL '7 days' THEN 5
      ...
      OR EXISTS (
        ...
        WHERE LENGTH(w.search_word) > 1
          AND (
            ...
            OR (l.attributes IS NOT NULL AND LOWER(l.attributes::text) LIKE '%' || w.search_word || '%')
          )
      )
    )
