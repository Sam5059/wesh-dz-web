/*
  # Correction du filtre de catégorie avec support des sous-catégories

  ## Problème
  Quand on clique sur "Véhicules", aucune annonce n'apparaît car les annonces
  sont liées aux sous-catégories (Voitures) et non aux catégories parentes (Véhicules).

  ## Solution
  Modifier le filtre `category_filter` pour inclure :
  - Les annonces directement liées à la catégorie
  - Les annonces liées aux sous-catégories de cette catégorie

  ## Changements
  - Remplacer `l.category_id = category_filter`
  - Par une recherche qui inclut les sous-catégories
*/
...
-- Créer la fonction avec support des sous-catégories
...
BEGIN
    ...
    CASE
      ...
      ELSE
        ...
        (
          ...
          -- Score description
          CASE
            WHEN LOWER(l.description) LIKE '%' || LOWER(search_term) || '%' THEN 20
            ELSE 0
          END
          +
          -- Score brand/model dans attributs
          CASE
            WHEN l.attributes->>'brand_name' IS NOT NULL
              AND LOWER(l.attributes->>'brand_name') = LOWER(search_term) THEN 90
            WHEN l.attributes->>'brand_name' IS NOT NULL
              AND LOWER(l.attributes->>'brand_name') LIKE '%' || LOWER(search_term) || '%' THEN 40
            WHEN l.attributes->>'model_name' IS NOT NULL
              AND LOWER(l.attributes->>'model_name') = LOWER(search_term) THEN 85
            WHEN l.attributes->>'model_name' IS NOT NULL
              AND LOWER(l.attributes->>'model_name') LIKE '%' || LOWER(search_term) || '%' THEN 35
            ELSE 0
          END
          +
          -- Score pour recherche multi-mots (chaque mot qui match)
          (
            SELECT COALESCE(SUM(
              CASE
  ...
  WHERE
    ...
    -- ✅ CORRECTION ICI : Inclure les sous-catégories
    AND (
